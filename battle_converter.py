...
class BattleConverter:
    def __init__(self, cfg):
        ...
        # move features
        self.move_feats = [
            BattleOptions('acc', 0, 100, 1, lambda x: x.accuracy, cfg.BATTLE.MOVE.ACCURACY),
            BattleOptions('bsp', 0, 200, 1, lambda x: x.base_power, cfg.BATTLE.MOVE.BASE_POWER),
            ...
            BattleOptions('pri', 0, 14, 1, lambda x: x.priority + 7, cfg.BATTLE.MOVE.PRIORITY),
            BattleOptions('cat', 0, 1, len(MOVE_CATS), lambda x: np.eye(len(MOVE_CATS))[MOVE_CATS[x.category]], cfg.BATTLE.MOVE.CAT),
            BattleOptions('type', 0, 1, len(TYPES), lambda x: np.eye(len(TYPES))[TYPES[x.type]], cfg.BATTLE.MOVE.TYPE)
        ]
        # pokemon features
        self.poke_feats = [
            BattleOptions('active', 0, 1, 1, lambda x: int(x.active), cfg.BATTLE.POKEMON.ACTIVE),
            BattleOptions('atk', 0, 2000, 1, lambda x: x.stats['atk'], cfg.BATTLE.POKEMON.ATTACK),
            BattleOptions('def', 0, 2000, 1, lambda x: x.stats['def'], cfg.BATTLE.POKEMON.DEFENCE),
            BattleOptions('spa', 0, 2000, 1, lambda x: x.stats['spa'], cfg.BATTLE.POKEMON.SPECIAL_ATTACK),
            BattleOptions('spd', 0, 2000, 1, lambda x: x.stats['spd'], cfg.BATTLE.POKEMON.SPECIAL_DEFENCE),
            BattleOptions('spe', 0, 2000, 1, lambda x: x.stats['spe'], cfg.BATTLE.POKEMON.SPEED),
            BattleOptions('hp', 0, 2000, 1, lambda x: x.current_hp if x.current_hp else 0, cfg.BATTLE.POKEMON.HP),
            BattleOptions('hp_frac', 0, 1, 1, lambda x: x.current_hp_fraction, cfg.BATTLE.POKEMON.HP_FRACTION),
            BattleOptions('att', 0, 1, 1, lambda x: int(x.fainted), cfg.BATTLE.POKEMON.FAINTED),
            BattleOptions('gender', 0, 1, len(GENDERS), lambda x: np.eye(len(GENDERS))[GENDERS[x.gender]], cfg.BATTLE.POKEMON.GENDER),
            BattleOptions('type1', 0, 1, len(TYPES), lambda x: np.eye(len(TYPES))[TYPES[x.types[0]]], cfg.BATTLE.POKEMON.TYPE1),
            BattleOptions('type1', 0, 1, len(TYPES), lambda x: np.eye(len(TYPES))[TYPES[x.types[0] if len(x.types) > 1 else TYPES['NULL']]], cfg.BATTLE.POKEMON.TYPE2),
        ]
        self.num_moves = cfg.BATTLE.POKEMON.NUM_MOVES # this will always be 4, basically
        # team options
    ...
    def _extract_poke(self, poke_obj):
        ...
        for m in poke_obj.moves:
            vals += self._extract_move(poke_obj.moves[m])
            move_counter += 1
    ...
    def battle_to_tensor(self, battle):
        ...
        for poke in battle.team:
            our_team += self._extract_poke(battle.team[poke])
        p_count = 0
        their_team = []
        for poke in battle.opponent_team:
            p_count += 1
            their_team += self._extract_poke(battle.opponent_team[poke])
